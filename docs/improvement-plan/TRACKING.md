...
| WS-06 | Editor-UX     | 8, 11, 12, 21                    | 1/2   | 🟨     | Claude      | 12 ✅ 8 ✅ 11 ✅ + error-boundary(19) ✅; 21 deferred (Phase 2); CR-2/CR-3 done                                                                                                                                                                                                                                                                                                                                              |
| WS-07 | Block-Styling | 15                               | 1     | ✅     | Claude      | color schema widened (hex3/4/8, rgb/rgba, transparent) + button radius/padding/border + url validation + container border width/style + CR-4 darkColor dedupe ✅; WS-03 VML intact; editor alpha-picker → CR-6; 15-D per-corner border-radius (shared `BORDER_RADIUS_SCHEMA`/`getBorderRadius`, EmailLayout+Container) + Spacer backgroundColor + editor `transparent` shortcut + widened editor EmailLayout color schema ✅ |
...
- [x] **8** — Undo / redo (WS-06)
- [x] **11** — Inline canvas editing (WS-06, Phase 2)
...
| ---------- | ------------- | ---------------------------------------------------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-06-26 | WS-05         | `email-builder/src/renderers/renderToText.ts`                          | WS-08 (`block-kit`) | **CR-1:** replace WS-05's stopgap local HTML tag-stripper with the shared `htmlToText` now exported from `@usewaypoint/block-kit/sanitize`. Low-risk follow-up.                                                                                                                                                                                                  | ✅ done (2026-06-29) — shared `htmlToText` widened to a superset via `{ preserveLinks, blockSeparator }` opts (defaults keep block-text's contract); `renderToText` now delegates, local stripper deleted. Both suites green + new option tests. |
| 2026-06-26 | WS-05         | `examples/.../App/TemplatePanel/`                                      | WS-06               | **CR-2:** wire `renderToText` into a new editor "Text" tab beside HTML/JSON.                                                                                                                                                                                                                                                                                     | ✅ done (2026-06-29) — new `TextPanel` + "Plain-text output" tab (`SubjectOutlined`) between HTML and JSON; `selectedMainTab` union + `MainTabsGroup` extended; inherits canvas error boundary. Example `tsc --noEmit` clean.                    |
| 2026-06-26 | WS-08         | `examples/.../InspectorDrawer/.../ImageSidebarPanel.tsx`, Avatar panel | WS-06               | **CR-3:** add inspector control for the new `decorative` boolean (Image/Avatar) + warn when an image has no `alt` and isn't decorative. Schema field + render behavior already shipped in the blocks.                                                                                                                                                            | ✅ done (2026-06-29) — `BooleanInput` "Decorative (hide from screen readers)" on both panels; Image shows a warning `Alert` when not decorative + alt empty. Example `tsc --noEmit` clean.                                                       |
